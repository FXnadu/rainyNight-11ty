...
- 构建脚本：scripts/*（日期补全、分类元数据同步、CSS 压缩、性能自检等）
...
A --> K["scripts/optimize-css-safe.js<br/>CSS 压缩"]
A --> L["scripts/perf-self-check.js<br/>性能自检"]
A --> M["scripts/clean-site.js<br/>清理输出目录"]
```
...
- npm 脚本与生命周期
  ...
  - build：按序执行 clean:site → sync-meta → eleventy → optimize-css-safe → perf-self-check
  - start：eleventy --serve 启动本地开发服务器
  - debug：DEBUG=* eleventy 输出详细日志
- Eleventy 配置
  - 插件：语法高亮、Mermaid、Markdown-it 扩展
  - 过滤器：日期、标题格式化
  - 集合：文章、分类树、分类详情页、按文件夹分组
...
- [scripts/sync-category-meta.js:204](file://scripts/sync-category-meta.js#L204)
- [.eleventy.js:36-181](file://.eleventy.js#L36-L181)
...
- 生命周期与依赖
  ...
  - build：严格顺序执行 clean → sync-meta → eleventy → optimize-css-safe → perf-self-check
...
- 关键点
  ...
  - optimize-css-safe 与 perf-self-check 作为构建收尾，保障产物质量
...
- npm 脚本依赖链
  ...
  - build 串行依赖：clean-site → sync-category-meta → eleventy → optimize-css-safe → perf-self-check
- Eleventy 配置依赖
...
## 结论
本项目通过 npm 脚本串联预处理、Eleventy 构建与后处理，形成稳定的构建生命周期。Eleventy 配置在数据层与模板层协同工作，借助全局数据推导与集合体系，实现内容与页面的自动化装配。配合性能自检与 CSS 压缩，能够持续保障产物质量。建议在 CI 中探索并行化与缓存策略以进一步提升效率。
